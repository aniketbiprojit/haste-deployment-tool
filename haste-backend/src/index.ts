import { PersistentStore } from './persist'
...
const store = new PersistentStore('master-thread')
...
const secret = process.env.SECRET_KEY ?? randomUUID()
...
app.get('/', async (req, res) => {
	...
	if (checkKeys(['redirect', 'auth', 'code'], req.query)) {
		...
		if (!execution_data) {
			...
			const token = sign({ ...execution_data, email }, secret)
